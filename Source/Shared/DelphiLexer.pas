...
var
  C: WideChar;
begin
  Utf8ToUnicode(@C, 1, P, UTF8CharSize(P[Index]));
...
begin
  ...
  begin
    Len := Length(Result);
    SetLength(Result, Len + FBufCount);
...
begin
  ...
  else if (ch1 in IdentFirstChars) {$IFDEF UNICODE}or ((P[0] >= #128) and IsUtf8Letter(P, 0)){$ENDIF} then
  begin
    // identifier
